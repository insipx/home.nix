...
-- ins_left {'chadtree', cond = conditions.buffer_not_empty, color = {fg = colors.magenta, gui = 'bold'}}
ins_left({ "fugitive", cond = conditions.buffer_not_empty, color = { fg = colors.magenta, gui = "bold" } })

ins_left({ "filename", path = 1, cond = conditions.buffer_not_empty, color = { fg = colors.magenta, gui = "bold" } })

ins_left({ "filetype", cond = conditions.buffer_not_empty, color = { fg = colors.violet, gui = "bold" } })
	...
	function()
		...
		for _, client in ipairs(clients) do
			...
			if filetypes and vim.fn.index(filetypes, buf_ft) ~= -1 then
				local lsp_name = string.lower(client.name)
